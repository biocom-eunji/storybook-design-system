...
// TextField(multiline=true)로 구현. 토큰 구조는 TextField와 동일하며
// 레이아웃만 다중행 전용 값을 사용.
...
export const CharCounter: Story = {
  ...
  render: () => (
    <View style={{ gap: spacing['3xlarge'], maxWidth: 400 }}>
      <Section
        title="글자 수 카운터"
        description="TextArea는 내부 하단 오른쪽에 글자 수 카운터가 표시됩니다."
      >
        <View style={{ gap: spacing.xlarge }}>
          <Col gap={spacing.small}>
  ...
  render: () => {
    ...
    return (
      <View style={{ gap: spacing['3xlarge'] }}>
        <Section
          title="디자인 스펙"
          description="Figma 시맨틱 토큰 기준 TextArea 상태별 스펙입니다."
          badge="디자인"
        >
          {states.map(state => {
...
export const Usage: Story = {
  ...
  render: () => (
    <View style={{ gap: spacing['3xlarge'] }}>
      <Section
        title="사용 가이드"
        description="TextArea는 TextField 컴포넌트의 multiline 모드입니다."
        badge="개발"
      >
        <CodeBlock
          title="Import (TextField와 동일)"
